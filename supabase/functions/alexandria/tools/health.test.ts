...
Deno.test("formatBodyCompSummary returns 'No entries' message for empty array", () => {
  ...
  assertEquals(
    summary,
    "No body composition entries found in the selected period.",
  );
});
...
Deno.test("formatBodyCompSummary includes latest metrics in output", () => {
  ...
  const summary = formatBodyCompSummary(entries as any, [], {
    from: "2026-06-01",
    to: "2026-06-07",
  });
...
// ============================================================================
// T10 Health Core Unit Tests: Validation, Normalization, Range & Lifecycle
// ============================================================================
...
function createMockServer() {
  ...
  registerHealthTools(mockServer, () => ({ method: "key", userId: "test-user-id" }));
  return { tools };
}

Deno.test("registerHealthTools registers all 9 health tools on server", () => {
  const { tools } = createMockServer();
  assertExists(tools["log_health"]);
  assertExists(tools["query_health"]);
  assertExists(tools["search_health"]);
  assertExists(tools["health_summary"]);
  assertExists(tools["refresh_summary"]);
  assertExists(tools["delete_health_entry"]);
  assertExists(tools["bodycomp_summary"]);
  assertExists(tools["source_coverage_report"]);
  assertExists(tools["coverage_transition_report"]);
});

Deno.test("log_health rejects invalid entry types and malformed shapes", () => {
  // Invalid entry type
  try {
    normalizeHealthEntry({
      entry_type: "unknown_metric",
      timestamp: "2026-09-12T10:00:00Z",
    });
    throw new Error("Should have failed");
  } catch (err: any) {
    assertEquals(err.message.includes("unsupported entry_type"), true);
  }
  ...
  try {
    ...
    assertEquals(
      err.message.includes("timestamp must be ISO-8601 with explicit offset"),
      true,
    );
  }

  // Non-number numeric_value
  try {
    normalizeHealthEntry({
      entry_type: "steps",
      timestamp: "2026-09-12T10:00:00Z",
      numeric_value: "five thousand" as any,
    });
    throw new Error("Should have failed");
  } catch (err: any) {
    assertEquals(
      err.message.includes("numeric_value must be a number or null"),
      true,
    );
  }
});

Deno.test("log_health canonical normalizer preserves aliases and assigns identity", () => {
  const normalized = normalizeHealthEntry({
    entry_type: "steps",
    timestamp: "2026-09-12T10:00:00Z",
    numeric_value: 5000,
    value: { count: 5000 },
    external_id: "ext-steps-123",
  });

  assertEquals(normalized.entry_type, "steps");
  assertEquals(normalized.numeric_value, 5000);
  assertExists(normalized.identity);
  assertEquals(normalized.contract_version, "health.v1");
});

Deno.test("query_health handler rejects event_from greater than event_to", async () => {
  const { tools } = createMockServer();
  const tool = tools["query_health"];

  const res = await tool.handler({
    event_from: "2026-09-15T12:00:00Z",
    event_to: "2026-09-10T12:00:00Z",
  });
  assertEquals(res.isError, true);
  assertEquals(
    res.content[0].text.includes(
      "event_from must be less than or equal to event_to",
    ),
    true,
  );
});

Deno.test("health_summary handler rejects from greater than to", async () => {
  const { tools } = createMockServer();
  const tool = tools["health_summary"];
  ...
  assertEquals(res.isError, true);
  assertEquals(
    res.content[0].text.includes("from date must be less than or equal to to date"),
    true,
  );
});

Deno.test("refresh_summary handler rejects invalid date format and invalid ranges", async () => {
  const { tools } = createMockServer();
  const tool = tools["refresh_summary"];

  const resInvalidDate = await tool.handler({
    date: "12-09-2026",
  });
  assertEquals(resInvalidDate.isError, true);
  assertEquals(
    resInvalidDate.content[0].text.includes("Invalid date format"),
    true,
  );

  const resInverted = await tool.handler({
    from: "2026-09-20",
    to: "2026-09-10",
  });
  assertEquals(resInverted.isError, true);
  assertEquals(
    resInverted.content[0].text.includes("from date must be less than or equal to to date"),
    true,
  );
});
