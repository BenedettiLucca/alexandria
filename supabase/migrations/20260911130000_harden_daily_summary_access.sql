-- Migration: 20260911130000_harden_daily_summary_access.sql
-- T25 BLOCKER 1 (P0): compute_daily_summary (SECURITY DEFINER) aceiteva
-- p_user_id de caller anon. Guarda interna: JWT so targeta a si mesmo.
-- Corpo = pg_get_functiondef real + guarda. Idempotente.
CREATE OR REPLACE FUNCTION public.compute_daily_summary(target_date date, p_user_id uuid DEFAULT NULL::uuid, p_timezone text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY INVOKER
...
BEGIN
    ...
    -- T25 BLOCKER 1: caller JWT-authenticated so pode targetar a si mesmo.
    IF COALESCE(current_setting('request.jwt.claims', true), '') <> ''
       AND p_user_id IS NOT NULL
       AND COALESCE(current_setting('request.jwt.claims', true), '')::jsonb->>'role' <> 'anon'
       AND COALESCE(current_setting('request.jwt.claims', true), '')::jsonb->>'sub' IS NOT NULL
       AND (current_setting('request.jwt.claims', true))::jsonb->>'sub' <> p_user_id::text THEN
      RAISE EXCEPTION 'Owner invariant violation: authenticated caller cannot compute summary for another owner';
    END IF;
